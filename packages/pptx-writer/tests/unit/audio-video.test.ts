...
describe('Audio & Video Serializers (Unit Tests)', () => {
  it('serializes audio element to DrawingML pic element', () => {
    ...
    expect(audioNode.tag).toBe('p:pic');
    const xml = renderXml(audioNode);
    expect(xml).toContain('r:link="rIdAudioLink"');
    expect(xml).toContain('r:embed="rIdMediaEmbed"');
    expect(xml).toContain('r:embed="rIdPosterImage"');
  ...
  it('serializes video element to DrawingML pic element', () => {
    ...
    expect(videoNode.tag).toBe('p:pic');
    const xml = renderXml(videoNode);
    expect(xml).toContain('r:link="rIdVideoLink"');
    expect(xml).toContain('r:embed="rIdMediaEmbed"');
    expect(xml).toContain('r:embed="rIdPosterImage"');
    expect(xml).toContain('<p14:media');
  ...
  it('builds slide timing with animations, automatic media, and onClick media', () => {
    ...
    expect(timing?.tag).toBe('p:timing');
    const xml = renderXml(timing);
    expect(xml).toContain('<p:timing>');
    expect(xml).toContain('<p:tnLst>');
    expect(xml).toContain('<p:par>');
    expect(xml).toContain('nodeType="mainSeq"');
    expect(xml).toContain('nodeType="interactiveSeq"');
  ...
  it('builds slide timing with start and end trimming', () => {
    ...
    expect(xml).toContain('st="2000"');
    expect(xml).toContain('end="12000"');
    expect(xml).toContain('vol="100000"');
  });
