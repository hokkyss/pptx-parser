...
- **`@monorepo/common`**: Shared types, DTOs, Zod schemas, and business constants (e.g. `pagination.dto.ts`, `logger`).
- **`@monorepo/utils`**: Pure, domain-agnostic utility functions (e.g., date formatting helpers, string manipulators).
...
import { logger } from '@monorepo/common/logger';
import { paginationDto } from '@monorepo/common/dto/pagination';
